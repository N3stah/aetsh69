...
async def build_live_context(db: AsyncSession, user_context: str = "CURRENT USER CONTEXT:\n  - Status: Guest (Not logged in)") -> str:
    ...
    try:
        async with db.begin_nested():
            result = await db.execute(text("SELECT p.name, p.slug, p.price_kes, c.name as category_name, p.stock_quantity FROM products p LEFT JOIN product_categories c ON p.category_id=c.id WHERE p.stock_quantity > 0 ORDER BY p.created_at DESC LIMIT 15"))
            products = result.fetchall()
        if products:
            ctx.append("\nSHOP INVENTORY (in stock):")
            for p in products:
                ctx.append(f"  - {p.name} (Slug: {p.slug}): KSh {p.price_kes} ({p.category_name}) — {p.stock_quantity} in stock")
        else:
